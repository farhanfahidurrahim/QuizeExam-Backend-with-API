...
@section("body")
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{route("quiz.topics.store")}}">
                        @csrf
                        <div class="row">
                            <div class="col-sm-4">
                    ...
                    @foreach($data as $key=>$subjects)
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <a class="badge bg-danger" href="{{route("quiz.topics.delete",[$subjects->id])}}">Delete</a>
                                ...
                                <div class="modal fade text-left" id="backdrop{{$subjects->id}}" tabindex="-1" role="dialog"
                                    ...
                                    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
                                        <div class="modal-content">
                                            ...
                                            <form method="POST" action="{{route("quiz.topics.edit",[$subjects->id])}}">

                                                @csrf
                                                <div class="modal-body">
                                                    <select class="form-control" name="quiz_subject">
                                                        @foreach($subject as $sub)
                                                            <option value="{{$sub->id}}" @if($sub->id==$subjects->quiz_subject_id) {{$value="selected"}} @endif>{{$sub->quiz_subject}}</option>
                                                        @endforeach
                                                    </select>
                                                </br>
